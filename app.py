...
def hello_world():
    if request.method == 'POST':
        ...
        count_obj = 0
        for i in s3.Bucket(config.custombucket).objects.all():
            count_obj += 1
        file_name = "file-id-" + str(count_obj + 1)

        try:
            s3.Bucket(config.custombucket).put_object(Key=file_name, Body=file_body)
...
def hello():
    try:
        statuscode = urllib.request.urlopen(config.kapp).getcode()
        if statuscode == 200:
